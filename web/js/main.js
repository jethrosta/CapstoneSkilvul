document.addEventListener('DOMContentLoaded', function() {
    ...
    var ocrText = document.getElementById('ocrText');  // Assuming you have an element to display OCR text
    var predictionText = document.getElementById('predictionText'); // Assuming you have an element to display prediction
        ...
        uploadButton.addEventListener('click', function() {
            if (fileInput.files.length > 0) {
                ...
                .then(data => {
                    ...
                    ocrText.textContent = JSON.stringify(data.text, null, 2); // Display OCR text
                    processedImage.src = data.processed_image;
                    resultDiv.style.display = 'block';
                    // predictionText.textContent = data.prediction; // Display prediction
                    // Set the prediction text
                    predictionText.innerHTML = data.prediction;

                    // Check if prediction text contains 'Eligible' or 'Not Eligible'
                    if (data.prediction.includes('Not Eligible')) {
                        // Add the 'green' class
                        predictionText.classList.add('red');
                    } else {
                        // Add the 'red' class
                        predictionText.classList.add('green');
                    }
                })
                .catch(error => {
